...
## Concerning floor and ceiling effects 

```{r}

robust_ate.ideo.q1 <- ictreg(direct ~ ideo_con + ideo_lib + income + men + college + age, treat = "treat_f", 
                 J = 3, 
         data = test, 
         floor = TRUE, 
         ceiling = TRUE,
...
robust_ate.par.q1 <- ictreg(direct ~ party_con + party_lib + income + men + college + age, treat = "treat_f", 
                 J = 3, 
         data = test, 
         floor = TRUE, 
         ceiling = TRUE,
...
robust_ate.ideo.q2 <- ictreg(indirect ~ ideo_con + ideo_lib + income + men + college + age, treat = "treat_f", 
                 J = 3, 
         data = test, 
         floor = TRUE, 
         ceiling = TRUE,
...
robust_ate.par.q2 <- ictreg(indirect ~ party_con + party_lib + income + men + college + age, treat = "treat_f", 
                 J = 3, 
         data = test, 
         floor = TRUE, 
         ceiling = TRUE,
...
robust_qd1 <- analyze_multi_ate(robust_ate.par.q1) %>%
  plot_multi_ate() +
  labs(title = "Checking floor and ceiling effects", 
       subtitle = "Partisanship",
       tags = "A")+
  scale_x_discrete(labels = c("Liberal", "Income", "Male", "Age", "College", "Conservative"))


robust_qd2 <- analyze_multi_ate(robust_ate.ideo.q1) %>%
  plot_multi_ate() +
  labs(title = "", 
       subtitle = "Ideology",
       tags = "B")+
...
robust_qid1 <- analyze_multi_ate(robust_ate.par.q2) %>%
  plot_multi_ate() +
  labs(title = "Checking floor and ceiling effects", 
       subtitle = "Partisanship",
       tags = "A")+
...
robust_qid2 <- analyze_multi_ate(robust_ate.ideo.q2) %>%
  plot_multi_ate() +
  labs(title = "", 
       subtitle = "Ideology",
       tags = "B")+
...
robust_qd1 / robust_qd2 
ggsave(here("outputs", "robust_multi_direct.png"))

robust_qid1 / robust_qid2
ggsave(here("outputs", "robust_multi_indirect.png"))
