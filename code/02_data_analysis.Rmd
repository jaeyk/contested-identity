  ...
  diff_summary <- data %>%
    ...
    summarise_each(
      funs(
        ...
        # Calculating confidence intervals
        conf_t1 = (MKinfer::boot.t.test(.[treat == 2], .[treat == 1], R = 100)$boot.conf.int[2] - MKinfer::boot.t.test(.[treat == 1], .[treat == 1], R = 100)$boot.conf.int[1]) / 2,
        conf_t2 = (MKinfer::boot.t.test(.[treat == 3], .[treat == 1], R = 100)$boot.conf.int[2] - MKinfer::boot.t.test(.[treat == 1], .[treat == 1], R = 100)$boot.conf.int[1]) / 2,
        conf_t3 = (MKinfer::boot.t.test(.[treat == 4], .[treat == 1], R = 100)$boot.conf.int[2] - MKinfer::boot.t.test(.[treat == 1], .[treat == 1], R = 100)$boot.conf.int[1]) / 2,
        conf_t4 = (MKinfer::boot.t.test(.[treat == 5], .[treat == 1], R = 100)$boot.conf.int[2] - MKinfer::boot.t.test(.[treat == 1], .[treat == 1], R = 100)$boot.conf.int[1]) / 2
      ),
      direct, indirect
    )
...
### 4.2. Recalculating CIs for CATE
